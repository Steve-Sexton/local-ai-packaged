...
A Docker Compose-based self-hosted AI development environment combining n8n (low-code automation), PostgreSQL (database), Ollama (local LLMs), Open WebUI (chat interface), Flowise (AI agent builder), Qdrant (vector store), Neo4j (graph database), SearXNG (search), Langfuse (observability), and Caddy (reverse proxy).
...
- **start_services.py**: Main entry point - generates SearXNG secret, checks SearXNG first-run settings, and starts local AI services
- **docker-compose.yml**: Main compose file with all local AI services. Uses YAML anchors for service templates (`x-n8n`, `x-ollama`, `x-init-ollama`)
...
- n8n connects to PostgreSQL at host `postgres`
